import { FileMeta, HookCb, Uploader, UploadHandler, UploadHandlerConstructor, UploadHook } from '../../src';
...
class CustomUploadHandler extends UploadHandler<CustomUploadHook> {
  upload():Promise<FileMeta[]>{
    return new Promise(resolve => {
      ...
      resolve([{ 'name':'aaa.xlsx', 'ext':'xlsx', 'size':100, 'url': 'oss/3423dfa.xlsx' }])
    })
  }
  about(){}
  process(){return 0}
...
describe('Uploader.ts', () => {
  ...
  it('upload hook', async () => {
    ...
    expect(fn1).toBeCalledWith([{ 'name':'aaa.xlsx', 'ext':'xlsx', 'size':100, 'url': 'oss/3423dfa.xlsx' }], uploader)
  });
