...
<template>
  <div class="rosterContent">
            <div class="r-title"> 
                <el-button v-show="showUpload" plain class="r-t-btn" v-on:click="backend">返回</el-button>
                班级小管家文件导入
            </div>
            ...
            <div v-show="showTips" v-cloak>
                <div class="rosterTips">
                    ...
                    <span class="roster-tips red">Excel表格中，必须包含有 [姓名] 列，如下图</span>
                    <span class="roster-tips">Excel可增加 [手机号] 列，可以一键短信邀请学生加入班级</span>
                </div>
                    <img src="https://allsystemfile.oss-cn-beijing.aliyuncs.com/miniprogram/bjxgj/ai/excel01.png" class="image">
                <div class="bottom ">
                    <el-button type="primary" class="button" v-on:click="startUpload">开始导入</el-button>
                </div>
            </div>
            ...
            <div class="rosterContent" v-cloak>
                ...
                <div class="el-upload__tip" slot="tip">只能上传.xls,.xlsx文件，且不超过10M</div>
                </el-upload>

                <div class="bottom ">
                    <el-button type="primary" class="button" :disabled="disabled" v-on:click="saveBtn">同步到班级小管家</el-button>
                </div>
            </div>

            <div class="r-footer">温馨提示：<a href="https://mp.weixin.qq.com/s/FCJnY78LclvvipYIhxNAzw" target="_blank">点击查看帮助文档</a></div>
        </div>
</template>
...
export default {
  ...
  methods:{
            ...
            login:async function(){
                ...
                //验证传输码是否有效
                try {
                    let result = await  axios.post(host+'/getTransCodeContent',{trans_code})
                        //console.log(result)
            ...
            uploadFile: async function(param){
               ...
               let ext = filename.split(".")[filename.split(".").length-1]
                ...
                try {
                    //验证传输码是否有效
                    let filePath = await  axios.post(host+'/getTransCodeContent',{trans_code:_this.input})
                        if(filePath.data.code!=0 || !filePath.data.data){
                           // _this.$message.error("传输码已过期，请重新输入");
                           _this.$confirm('传输码已过期，请重新输入', '提示', {
                    ...
                    if(result.status==200){
                       ...
                       addEvent({
                        ...
                        _this.$message.error("导入文件出现异常，请重新上传");
                    }
            ...
            //更新文件地址到Redis
             saveBtn: async function(){
                ...
                if(!filesStr){
                    this.$message.error('请先完成文件上传或者检查传输码！');
                    return
                }
                //验证传输码是否有效
                let filePath = await  axios.post(host+'/getTransCodeContent',{trans_code:that.input})
                    if(filePath.data.code!=0 || !filePath.data.data){
                        that.$message.error("传输码已过期，请重新输入");
                        return
                    }
                //同步文件信息
                axios.post(host+"/setTransCodeContent",{trans_code:that.input,files:filesStr}).then(res=>{
                    console.log(res)
                    if(res.data.code==0){
                       
                            this.$confirm('导入完毕，请到小程序中查看结果', '操作成功', {
                                confirmButtonText: '确定',
                                showCancelButton:false,
                                type: 'success'
                                }).then(() => {
                        ...
                        this.$message.error('文件信息更新失败，请重试！');
                    }
...
.roster-tips:before {
    ...
    border: 1px solid rgba(0, 154, 225, .1);
    border-radius: 12px;
    margin-right: 10px;
    vertical-align: top;
}
